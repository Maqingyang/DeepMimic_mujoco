...
class Dset_mocap(object):
    def __init__(self, mocap_config, mocap_vel, randomize):
        self.mocap_config = np.array(mocap_config[:])
        self.mocap_vel = np.array(mocap_vel[:])
        assert len(self.mocap_config) == len(self.mocap_vel)
        self.randomize = randomize
        self.num_mocap = len(mocap_config)
        self.init_pointer()

    def init_pointer(self):
        self.pointer = 0
        if self.randomize:
            idx = np.arange(self.num_mocap)
            np.random.shuffle(idx)
            self.mocap_config = self.mocap_config[idx]
            self.mocap_vel = self.mocap_vel[idx]

    def get_next_batch(self, batch_size):
        # if batch_size is negative -> return all
        if batch_size < 0:
            return self.mocap_config, self.mocap_vel
        if self.pointer + batch_size >= self.num_mocap:
            self.init_pointer()
        end = self.pointer + batch_size
        mocap_config = self.mocap_config[self.pointer:end]
        mocap_vel = self.mocap_vel[self.pointer:end]
        self.pointer = end
