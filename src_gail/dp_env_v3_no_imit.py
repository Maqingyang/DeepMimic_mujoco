...
class DPEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def get_mass_center(self):
        ...
        return (np.sum(mass * xpos, 0) / np.sum(mass))[:2].copy()
    ...
    def step(self, action):
        ...
        xy_position_before = self.get_mass_center()
        self.do_simulation(action, step_times)
        xy_position_after = self.get_mass_center()

        xy_velocity = (xy_position_after - xy_position_before) / self.dt
        x_velocity, y_velocity = xy_velocity

        reward_alive = 0.1
        reward = x_velocity + reward_alive
