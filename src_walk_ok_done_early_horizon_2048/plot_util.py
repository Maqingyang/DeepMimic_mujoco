...
COLORS = ['blue', 'green', 'red', 'cyan', 'magenta', 'purple', 'pink',
        'brown', 'orange', 'teal',  'lightblue', 'lime', 'lavender', 'turquoise',
        'darkgreen', 'tan', 'salmon', 'gold',  'darkred', 'yellow', 'black',  'darkblue']
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        if average_group:
            for group in sorted(groups):
                ...
                if not any(xys):
                    continue
                color = COLORS[sorted(groups).index(group) % len(COLORS)]
                ...
                g2l[group] = l
                if shaded_err:
                    ax.fill_between(usex, ymean - ystderr, ymean + ystderr, color=color, alpha=.25)
                if shaded_std:
                    ax.fill_between(usex, ymean - ystd,    ymean + ystd,    color=color, alpha=.2)
                ## printing statistics
                # print(group, ymean, ystderr)
        ...
        if legend:
            ...
            od_g2l = OrderedDict(sorted(g2l.items()))
            if any(od_g2l.keys()):
                ax.legend(
                    od_g2l.values(),
                    ['%s (%i)'%(g, g2c[g]) for g in od_g2l] if average_group else g2l.keys(),
