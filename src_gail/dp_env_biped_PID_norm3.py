"""
action are normalized to [mean-1, mean+1], use deviation to scale the range
"""
#!/usr/bin/env python3
import numpy as np
import math
...
class DPEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def sample_1_expert_traj(self):
        ...
        ratio = 1 - (curr_time % self.mocap_dt) / self.mocap_dt
        pos_0 = lerp(self.mocap.data_config[idx_curr],self.mocap.data_config[idx_curr + 1],ratio).copy()
    ...
    def step(self, action):
        ...
        for ac_name, norm_ac in zip(actutor_seq, action):
            ...
            unnorm_action.append((norm_ac-(H+L)/2.)*(H-L)/2. + (H+L)/2.)
